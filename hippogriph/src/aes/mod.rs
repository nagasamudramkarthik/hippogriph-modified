...
/// Modified sub_bytes with fused encoding clone
fn sub_bytes(
    state: &AESStateArithmetic,
    server_key: &ServerKey,
    client_key_debug: &ClientKey,
) -> AESStateArithmetic {
    assert_eq!(state.nibbles.len(), 32);

    // Clone encoding once and reuse in the loop
    let encoding_clone = state.encoding.clone();
    let enc_vec = vec![encoding_clone.clone(); 2];

    // Perform full_tree_bootstrapping with pre-cloned encoding vector
    let nibbles = (0..16)
        .into_par_iter()    // uncomment to enable parallelism
        .map(|i| (i, state.nibbles[i*2..(i+1)*2].to_vec()))
        .map(|(i, v)| {
            server_key.full_tree_bootstrapping(
                &v,
                &enc_vec,
                256,
                &clear_sub_bytes,
